...
// Ability objects
  //Pool 1
  //0
  let speedBuffOnBuy = new Ability("Filler Name 1", "onBuy", speedBuffEffect, "When bought, buffs all other units with + 3 speed!", 0)
  let strengthDebuffOnDeathS1 = new Ability("Filler Name 2","stats", strengthDebuffOnDeathEffectS1, "\n- Has 1 health \n- On Death, debuffs all enemy strength stats by 25%", 1)
  let debuffImmunity = new Ability("Filler Name 3", "onDebuff", disableDebuff, "Can't be debuffed", 2)
  let debuffOnOutsped = new Ability("Filler Name 4", "onHurt", debuffOnOutspedEffect, "When hit by a faster unit, gains 2 strength and that unit loses its speed.",3)
  let copyStatsFromBehind = new Ability("Filler Name 5", "onAttack", copyStatsFromBehindEffect, "Before attacking copies the strength, speed and range from the unit behind it.", 4)
  //5
  let stealStatsOnKO = new Ability("Filler Name 6", "onKO", stealStatsOnKOEffect, "After KOing an enemy gain 1/2 of their strength and speed.", 5)
  let annoyingThing = new Ability("Filler Name 7", "onBattleStart", annoyingThingEffect, "At the start of each battle copies an opponent's unit that is in the mirrored column as this.", 6)
  let healthBuffAllOnTurnEnd = new Ability("Filler Name 8", "onTurnEnd", healthBuffAllEffect, "At the end of each turn buffs all units, without this ability, by 2 health.", 7)
  let reactivateOnBuysOnSell = new Ability("Filler Name 9", "onSell", reactivateOnBuysOnSellEffect, 'When deleted, activates all "When bought" abilities of your other units.', 8)
  let yoinkRangeOnBuy = new Ability("Filler Name 10", "onBuy", yoinkRangeOnBuyEffect, "When bought steals all other units' range stats and converts them to strength.", 9)
  //10
  let extraBuffs = new Ability("Filler Name 11", "onOtherBuff" , extraBuffsEffect, "When a unit is buffed give +1 of the stat it was buffed in.", 10)
  let generalBuffs = new Ability("Filler Name 12", "onBuy", generalBuffsEffect, "On buy, buff all other units by one of each stat.", 11)
  let stealBuffs = new Ability("Filler Name 13", "onOtherBuff", stealBuffsEffect, "When another team member is buffed in a stat other than health steal half of that buff.", 12)
  //Pool 2
  let stunOnDeath = new Ability("Filler Name 14", "onDeath", stunOnDeathEffect, "On death, the attacker is stunned (loses all range and it's ability) unil it takes a hit.", 13)
  ...
  let niceZombie = new Ability("Filler Name 15", "onBattleStart", niceZombieEffect, "Before battle, has a strength stat equal to the health of your opponents highest health unit.", 16)
  let freeStrength = new Ability("Filler Name 16", "onBuff", freeStrengthEffect, "On buff,  lose that buff but buff the left-most unit (unless this unit is the left-most) with this ability with 1 strength.", 17)
  ...
  let offensiveBuffer = new Ability("Filler Name 17", "onTurnEnd", offensiveBufferEffect, "On turn end, give the unit ahead +2 strength and speed if it has <15 of that stat respectively.", 19)
  //20
  let doubleStats = new Ability("Filler Name 18", "stats", doubleStatsEffect, "Has 2x the strength and health it normally would have.", 20)
  ...
  let soulEater = new Ability ("Filler Name 19", "onKO", soulEaterEffect, "After KOing a unit, gain their soul. Then release that soul, dealing the strength of the KO'ed enemy to the farthest enemy.", 22)
  let buy1Get1Free = new Ability("Filler Name 20", "onBuy", buy1Get1FreeEffect, "When bought, lets you buy another unit.", 23)
  ...
  //25
  let shield = new Ability("Shield", "onOpponentAttack", shieldEffect, "When an opponent attacks a unit, this unit absorbs the damage of that attack.", 25)
...
function strengthDebuffOnDeathEffectS2(unit) {
  var units = game.players[2 - unit.player.number].units
...
function debuffOnOutspedEffect(unit) {
  var otherUnit = game.players[2 - unit.player.number].units[0]
...
function stunOnDeathEffect (unit) {
  if (game.players[2 - unit.player.number].units.length == 0) {
    return
  }
  var otherUnit = game.players[2 - unit.player.number].units[0]
...
function niceZombieEffect (unit) {
  let opponent = game.players[2 - unit.player.number]
...
function summonerEffect(unit) {
  ...
  let opponent = game.players[2 - unit.player.number].units[0]
...
function soulReleaseEffect(unit) {
  ...
  let otherUnits = game.players[2 - unit.player.number].units
...
//Id: 25
function shieldEffect (enemy, unit, targets) {
  enemy.update("yay")
  unit.takeDamage(enemy.damage)
  for (let target of targets) {
    if (target.ability.when == "onHurt") {
      target.ability.effect(target, 0)
    }
  }
  field.getRange("A1").getValue()
